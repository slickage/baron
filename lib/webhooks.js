...
function postToWebhook(webhookObj, cb) {
  log.debug('[Webhook: ' + webhookObj.invoice_id + '] Calling webhook ' + webhookObj.url);
  var postData = {};
  ...
  postToWebhook(webhookObj, function(err) {
    if (err) {
      ...
      db.insert(webhookObj, function(err) {
        if (err) {
          log.error({ webhookObj: webhookObj }, '[Webhook Storage: ' + webhookObj.invoice_id + '] failed to store webhook for retry');
        }
        else {
          log.warn('[Webhook Failed: ' + webhookObj.invoice_id + '] failed to notify ' + webhookObj.url);
        }
      });
    }
    else {
      log.debug('[Webhook Success: ' + webhookObj.invoice_id + '] successfully notified ' + webhookObj.url);
    }
  });
}
...
var postToWebhookIgnoreFailure = function(webhookObj, cb) {
  postToWebhook(webhookObj, function(err) {
    if (err) {
      log.error('[Webhook Retry Failed: ' + webhookObj.invoice_id + '] failed to notify ' + webhookObj.url);
      cb();
    }
    else {
      db.destroy(webhookObj._id, webhookObj._rev, function(err) {
        if (err) {
          log.error({ webhookObj: webhookObj }, '[Webhook Destroy Failed: ' + webhookObj.invoice_id + '] failed to destroy');
        }
      });
      log.info('[Webhook Retry Success: ' + webhookObj.invoice_id + '] successfully notified ' + webhookObj.url);
      cb();
    }
  });
};
...
var determineWebhookCall = function(invoiceId, origStatus, newStatus) {
  if (origStatus !== newStatus) {
    db.findInvoice(invoiceId, function(err, invoice) {
      if (err) {
        log.error(err, '[Webhooks (' + invoiceId + ')] Error finding invoice.');
      }
